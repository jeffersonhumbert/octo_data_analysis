...
```{bash}
cairosvg timing.svg -o timing.png -d 100
```

```{bash}
inkscape timing.svg -o Figure2.eps --export-ignore-filters --export-ps-level=3
```
...
```{r}
sum(octos.present$duration[octos.present$number>1])/sum(octos.present$duration[octos.present$number>=1])
```
...
```
Finding the total number of hours that the cameras were deployed
```{r}
sum(octos.present$duration)/60
```
...
Setting up probabilities for chi-squared test
```{r}
reach.probs=c(16/(16+12),12/(16+12))
reach.probs
```

Running chi-squared test
```{r octo-res/nonres chi}
reach.chi=chisq.test(reach.tab)
reach.chi
```

### T-test of non-resident octopus visit duration
Testing my assumptions for T-Test
```{r}
...
```{bash}
inkscape allchi.svg -o Figure3.eps --export-ignore-filters --export-ps-level=3
```
...
Sampling 612 night times and 612 day times to see if an octopus is present
```{r}
...
```{r}
testcorr=rbind(daynight.samp.plot,c(NA,NA),perifort.plot,c(NA,NA),octo.res)
corrplot(testcorr,is.cor=F,tl.col="black",addCoef.col = "black",cl.pos=F,addgrid.col=NA,col.lim=c(-5,5),bg="#00000000")
```
```{r}
paste0("p=",signif(daynight.samp.chi$p.value,digits=2))
```


```{r}
svg(filename="chis.svg",width=7,height=15)
par(fig=c(0,1,0.1,0.95))
corrplot(testcorr,is.cor=F,tl.col="black",addCoef.col="black",addgrid.col=NA,col.lim=c(-5,5),
...
#for (i in 1:nrow(testcorr)){
#  for (j in 1:ncol(testcorr)){
#    if (!is.na(testcorr[i,j])){
#      text(j,7-i,signif(testcorr[i,j],digits=2),col="red")
#    }
#  }
#}

axis(1,at=c(1,2),lwd=0,labels = c("present","absent"),cex.axis=2,padj=3.5)
points(c(1,2,1,2),c(2,2,5,5),pch=21,bg="white",col="white",cex=3) # a real janky way to hide the questions marks
...
if (octo.chi$p.value>=0.001){
  text(1.5,1.4,paste0("p=",signif(octo.chi$p.value,digits=2)),cex=1.5)
...
```{bash}
cairosvg chis.svg -o chis.png -d 300
```

```{bash}
inkscape chis.svg -o Figure4.eps --export-ignore-filters --export-ps-level=3
```
