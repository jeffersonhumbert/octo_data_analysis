...
```{r}
kelpie.tab=
  as.vector(table(kelpie.visit$octopus,kelpie.visit$daynight))
kelpie.tab
```
...
kelpie.probs=
  c(
    c(min.absent/(min.absent+min.present),min.present/(min.absent+min.present))*mean(kelpie.visit$day_expected),
    c(min.absent/(min.absent+min.present),min.present/(min.absent+min.present))*(1-mean(kelpie.visit$day_expected))
  )
kelpie.probs
```
...
colnames(kelpie.res)=c("day","night")
rownames(kelpie.res)=c("octopus absent","octopus present")

corrplot(kelpie.res,is.cor=F,col.lim=c(-10,10))
```
...
```{r}
ling.tab=
  as.vector(table(ling.visit$octopus,ling.visit$daynight))
```
...
ling.probs=
  c(
    c(min.absent/(min.absent+min.present),min.present/(min.absent+min.present))*mean(ling.visit$day_expected),
    c(min.absent/(min.absent+min.present),min.present/(min.absent+min.present))*(1-mean(ling.visit$day_expected))
  )
```
...
colnames(ling.res)=c("day","night")
rownames(ling.res)=c("octopus absent","octopus present")

corrplot(ling.res,is.cor=F,col.lim=c(-10,10))
```
...
```{r}
rock.tab=
  as.vector(table(rock.visit$octopus,rock.visit$daynight))
```
...
rock.probs=
  c(
    c(min.absent/(min.absent+min.present),min.present/(min.absent+min.present))*mean(rock.visit$day_expected),
    c(min.absent/(min.absent+min.present),min.present/(min.absent+min.present))*(1-mean(rock.visit$day_expected))
  )
```
...
colnames(rock.res)=c("day","night")
rownames(rock.res)=c("octopus absent","octopus present")

corrplot(rock.res,is.cor=F,col.lim=c(-10,10))
```
...
```{r}
crab.tab=
  as.vector(table(crab.visit$octopus,crab.visit$daynight))
```
...
crab.probs=
  c(
    c(min.absent/(min.absent+min.present),min.present/(min.absent+min.present))*mean(crab.visit$day_expected),
    c(min.absent/(min.absent+min.present),min.present/(min.absent+min.present))*(1-mean(crab.visit$day_expected))
  )
```
...
colnames(crab.res)=c("day","night")
rownames(crab.res)=c("octopus absent","octopus present")
...
Assumptions fail, so I am using a permutation two-sample test.
```{r crab permTS}
permTS(crab.visit$duration[crab.visit$octopus=="absent"],crab.visit$duration[crab.visit$octopus=="present"],
       alternative="two.sided",method="exact.mc",control=permControl(nmc=10000))
...
Adding if octopuses are present or absent during each visit to my dataframe
```{r}
octo.visit$octopus="unknown"


for (i in 1:nrow(octos.present)){
  if (octos.present$number[i]>0){
    octo.visit$octopus[octo.visit$deployment==octos.present$`Deployment #`[i]&
                 octo.visit$time>octos.present$realtime[i]&
                 octo.visit$time<octos.present$endtime[i]]="present"
  }
  if (octos.present$number[i]==0){
    octo.visit$octopus[octo.visit$deployment==octos.present$`Deployment #`[i]&
                 octo.visit$time>octos.present$realtime[i]&
                 octo.visit$time<octos.present$endtime[i]]="absent"
  }
}

octo.visit=octo.visit[!octo.visit$octopus=="unknown",]
```
...
```{r}
octo.tab=
  as.vector(table(octo.visit$octopus,octo.visit$daynight))
```
...
octo.probs=
  c(
    c(min.absent/(min.absent+min.present),min.present/(min.absent+min.present))*mean(octo.visit$day_expected),
    c(min.absent/(min.absent+min.present),min.present/(min.absent+min.present))*(1-mean(octo.visit$day_expected))
  )
```
...
colnames(octo.res)=c("day","night")
rownames(octo.res)=c("octopus absent","octopus present")

corrplot(octo.res,is.cor=F,col.lim=c(-10,10))
```

Testing my assumptions for T-Test
```{r}
shapiro.test(octo.visit$duration[octo.visit$octopus=="present"])
shapiro.test(octo.visit$duration[octo.visit$octopus=="absent"])
bartlett.test(octo.visit$duration,octo.visit$octopus)
```

Assumptions fail, so I am using a permutation two-sample test.
```{r lingcod permTS}
octo.permTS=permTS(octo.visit$duration[octo.visit$octopus=="absent"],octo.visit$duration[octo.visit$octopus=="present"],
       alternative="two.sided",method="exact.mc",control=permControl(nmc=10000))
octo.permTS
```
...
Seeing what the mean difference is between lingcod visit duration when octopuses are absent versus octopuses are present.
```{r}
aggregate(duration~octopus,data=octo.visit,FUN="max")
```
Plotting the data.
```{r lingcod raw boxplot}
boxplot(duration~octopus,data=octo.visit,range=0)
```

This boxplot looks horrible so I am manually making boxplot with a broken Y-axis so the long tails don't obscure the main portion of the data.

```{r}
svg("Nonresident_octo_visitation_duration.svg",width=3,height=3,pointsize = 6)
par(fig=c(0,1,0.35,1))
boxplot(duration~octopus,data=octo.visit,range=0,ylim=c(2,9),axes=F,ylab="",xlab="")
axis(2,at=c(6,6.5))
box()

par(fig=c(0,1,0,0.65),new=T)
boxplot(duration~octopus,data=octo.visit,range=0,ylim=c(0,0.15),axes=F,ylab="",xlab="")
axis(2,at=c(0,0.15))
axis(1,at=c(1,2),label=c("octopus absent","octopus present"),cex.axis=1.5)
box()
...
```{r}
all.res=t(all.res)
colnames(all.res)=rep("",8)
sets=c("ling.visit","rock.visit","crab.visit")
```


```{r Interspecifc Interaction plot}
svg(filename="allchi.svg",width=7,height=7)
plot(c(0,1),c(0,1),type="n",axes=F,ylab="",xlab="")
...
if (kelpie.permTS$p.value<=0.05){
  ...
  text(1.5,exp(log(max(kelpie.visit$duration))+1),paste("p=",format(signif(kelpie.permTS$p.value,1),scientific = F),sep=""))

}


par(fig=c(start+box.next*1,start+box.next*1+box.width,0.22,.8),new=T)
...
par(fig=c(start+box.next*1,start+box.next*1+box.width,0.536,1),new=T)
...
if (ling.permTS$p.value<=0.05){
  ...
  text(1.5,exp(log(max(ling.visit$duration))+1),paste("p=",format(signif(ling.permTS$p.value,1),scientific=F),sep=""))
}


par(fig=c(start+box.next*2,start+box.next*2+box.width,0.22,.8),new=T)
...
par(fig=c(start+box.next*2,start+box.next*2+box.width,0.536,1),new=T)
...
if (rock.permTS$p.value<=0.05){
  ...
  text(1.5,exp(log(max(rock.visit$duration))+1),paste("p=",format(signif(rock.permTS$p.value,1),scientific=F),sep=""))
}


par(fig=c(start+box.next*3,start+box.next*3+box.width,0.22,.8),new=T)
boxplot(duration~octopus,data=crab.visit,range=0,ylim=c(0,bottom.max),axes=F,ylab="",xlab="")
box(lwd=3)

par(fig=c(start+box.next*3,start+box.next*3+box.width,0.536,1),new=T)
...
mtext(paste("n=",nrow(kelpie.visit),sep=""),side=3,line=n.line,cex=1,adj=0.09)
...
mtext(paste("n=",nrow(ling.visit),sep=""),side=3,line=n.line,cex=1,adj=0.37)
mtext("elongatus",side=3,line=species.line1,cex=1,adj=0.36,font=3)
mtext("Ophiodon",side=3,line=species.line2,cex=1,adj=0.36,font=3)
mtext(paste("n=",nrow(rock.visit),sep=""),side=3,line=n.line,cex=1,adj=0.63)
mtext("Sebastes sp.",side=3,line=species.line1,cex=1,adj=0.63,font=3)
mtext(paste("n=",nrow(crab.visit),sep=""),side=3,line=n.line,cex=1,adj=0.9)
mtext("Cancer productus",side=3,line=species.line1,cex=1,adj=0.97,font=3)
