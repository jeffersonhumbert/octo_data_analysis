...
for (i in 2:nrow(octos.present)){
  ...
  octos=c(octos,
          rep(octos.present$number[i],
          length(seq(from=octos.present$realtime[i],
                     to=octos.present$endtime[i],by="1 min"))))
}
...
```{r}
daynight.samp=c(sum(sample(octos[daynight=="night"]>0,612)),
                sum(sample(octos[daynight=="day"]>0,612)))
...
axis(1,at=c(1,2),lwd=0,labels = c("absent","present"),cex.axis=2,padj=3.5)
