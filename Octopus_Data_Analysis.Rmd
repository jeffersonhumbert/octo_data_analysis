...
to.plot=events$realtime[events$Behavior=="frame_arrival"&fish]
...
Plotting the data.
```{r}
boxplot(duration~octopus,data=kelpie.visit,range=0, ylim=c(0,1))
```
...
Assumptions fail, so I am using a permutation two-sample test.
```{r}
permTS(ling.visit$duration[ling.visit$octopus=="absent"],ling.visit$duration[ling.visit$octopus=="present"],
       alternative="two.sided",method="exact.mc",control=permControl(nmc=10000))
...
```{r}
aggregate(duration~octopus,data=ling.visit,FUN="mean")
```
Plotting the data.
```{r}
...
Plotting the data.
```{r}
boxplot(duration~octopus,data=rock.visit,range=0,ylim=c(0,0.5))
```
...
```{r}
svg(filename="allchi.svg")
corrplot(all.res,is.cor=F,col.lim=c(-10,10),tl.col="black",addCoef.col = "black",cl.pos=F)

for (i in c(0,2,4,6)){
  lines(c(.5,2.5,2.5,.5,.5),c(.5+i,.5+i,2.5+i,2.5+i,.5+i),lwd=3)  
}
text.x=2.7
text.x2=3
text.cex=1
