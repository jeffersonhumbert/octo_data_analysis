...
octoarrival.tab=data.frame(table(factor(hour(events$realtime[events$Behavior=="frame_arrival"&events$Species=="octopus_rubescens"]),levels=0:23)))
xaxis=seq(from=0,to=359,by=(360/nrow(octoarrival.tab)))*(pi/180)
```

```{r}
setrise=strptime("07/23/2021",format="%m/%d/%Y")

rays=seq(from=0,to=pi*2,length.out=10000)
col.rays=rep("lightcyan",length(rays))

day.start=round((as.numeric(difftime(sunriset(bay, as.POSIXct(setrise), direction="sunrise", POSIXct.out=TRUE)$time,setrise))/24)*length(rays))
day.end=round((as.numeric(difftime(sunriset(bay, as.POSIXct(setrise), direction="sunset", POSIXct.out=TRUE)$time,setrise))/24)*length(rays))
col.rays[day.start:day.end]="lightgoldenrodyellow"

radial.plot(rep(c(0,max(octoarrival.tab$Freq)),length(rays)/2),rays,show.grid=T,clockwise=T,start=pi/2,line.col=col.rays,
            label.pos=xaxis,labels=0:23,show.grid.labels = F)
radial.plot(octoarrival.tab$Freq,xaxis,labels=0:23,clockwise=T,show.grid.labels = F,rp.type="r",
            lwd=3,line.col="tomato3",start=pi/2,rad.col="white",grid.col="white",add=T)
...
#I just change "frame_arrival"&____ to include my specific species
hour.dec=hour(events$realtime[events$Behavior=="frame_arrival"&fish3])+
minute(events$realtime[events$Behavior=="frame_arrival"&fish3])/60
...
radial.plot(rep(c(0,1),length(rays)/2),rays,show.grid=T,clockwise=T,start=pi/2,line.col=col.rays,
            label.pos=xaxis,labels=0:23,show.grid.labels = F)
rose.diag(times,bins=24,col="tomato",shrink=0.4,add=T,axes=F,radii.scale="linear",prop=6)
points(times,pch=21,bg="tomato",cex=0.5)
